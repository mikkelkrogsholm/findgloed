...
export function WaitlistConfirmPage() {
  ...
  const [message, setMessage] = useState("Vi bekræfter din tilmelding...");
    ...
    async function run() {
      try {
        const response = await fetch(
          `${API_URL}/api/waitlist/confirm?token=${encodeURIComponent(token)}`,
          {
        ...
        if (response.ok && payload.ok && payload.status === "confirmed") {
          setState("success");
          setMessage(payload.message ?? "Din tilmelding er bekræftet.");
          return;
        }

        if (response.ok && payload.ok && payload.status === "already_confirmed") {
          setState("already_confirmed");
          setMessage(payload.message ?? "Din tilmelding er allerede bekræftet.");
          return;
        }
    ...
    <section className="mx-auto w-full max-w-3xl px-6 py-24">
      <Card className="p-8 md:p-10">
        <CardContent className="space-y-4 pt-0">
          <CardTitle>Bekræft din tilmelding</CardTitle>
